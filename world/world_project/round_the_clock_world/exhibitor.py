...
class Exhibitor(Exhibitor_super):
    ...
    def __init_exhibitor(self):
        ...
        class Point:
            ...
            def draw_night(self, night_surface):
                night_surface.fill((0, 0, 0, 115))
                self.exhibitor.window.blit(night_surface, (self.left, self.top))
    ...
    def display(self, mode="normal"):
        ...
        # 得到大环境 时间
        day_time = self.world.get_state().get_day_time()
        ...
        """
            画方格世界
        """
        self.draw_world(landform_map, water_map, terrain_map, animals_position, plants_position, objs_position, day_time)
    ...
    def draw_world(self, landform_map, water_map, terrain_map, animals_position, plants_position, objs_position, day_time):
        # 找到全地图的最高点
        ...
        # 画夜幕
        if day_time < 360 or day_time > 1080:
            night_surface = \
                self.pygame.Surface(
                    (self.world_win_size[1],
                     self.world_win_size[0])
                    , self.pygame.SRCALPHA, 32)
            self.Point(self, row=0, col=0).draw_night(night_surface)
